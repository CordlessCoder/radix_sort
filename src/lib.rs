            ...
            $(
                if $offset % 2 == 0 {
                    if T::BITS >= WIDTH * ($offset + 1) {
                        radix_sort_pass::<WIDTH, BUCKETS, { WIDTH * $offset }, T>(&mut counters, values, as_uninit_slice_mut(buf));
                    }
                } else if T::BITS >= WIDTH * ($offset + 1) {
                    radix_sort_pass::<WIDTH, BUCKETS, { WIDTH * $offset }, T>(&mut counters, buf, as_uninit_slice_mut(values));
                }
            )*
        }
...
pub fn radix_sort<T: RadixSortable>(values: &mut [T], buf: &mut [MaybeUninit<T>]) {
    ...
    if should_use_small_algo::<T>(values.len()) {
        ...
        let sort = radix_sort_unsigned_for_width!(16, [1, 2, 3, 4, 5, 6, 7]);
        sort(values, buf);
    }
